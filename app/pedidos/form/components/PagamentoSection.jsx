...
export default function PagamentoSection({ form, handleChange }) {
  ...
  const [despesas, setDespesas] = useState([{ descricao: 'Transporte', valor: 40 }, { descricao: 'Taxa de operação', valor: 5.99 }]);
  const [descontos, setDescontos] = useState([{ descricao: 'Desconto concedido', valor: 15 }, { descricao: 'Bônus fidelidade', valor: 10 }]);
  ...
  const salvarItem = () => {
    ...
    (novoItem.tipo === 'Despesa' ? setDespesas : setDescontos)(arr => [...arr, novaEntrada]);
    setNovoItem({ tipo: 'Despesa', descricao: '', valor: '' });
    setDialogOpen(false);
  };

  const excluirItem = (index, tipo) => (tipo === 'Despesa' ? setDespesas : setDescontos)(arr => arr.filter((_, i) => i !== index));
  ...
  const renderDialogPagamento = () => (
    <Dialog open={dialogPagamentoOpen} onClose={() => setDialogPagamentoOpen(false)} fullWidth maxWidth="sm">
      ...
      <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <TextField select label="Forma de pagamento" value={forma} onChange={(e) => setForma(e.target.value)} fullWidth>
          {['Pix', 'Dinheiro', 'Cartão', 'Transferência', 'Outro'].map(opcao => <MenuItem key={opcao} value={opcao}>{opcao}</MenuItem>)}
        </TextField>
        <Autocomplete options={sugestoesRecebedor} value={quemRecebeu} onChange={(e, val) => setQuemRecebeu(val)} renderInput={(params) => <TextField {...params} label="Recebido por" fullWidth />} freeSolo />
        <TextField label="Data do pagamento" type="date" value={dataPagamento} onChange={(e) => setDataPagamento(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
  ...
  return (
    <Box>
      <Tooltip title="Editar pagamento">
        <IconButton onClick={() => setOpen(true)} sx={{ borderRadius: '1.5rem', bgcolor: form.total ? 'success.main' : 'warning.main', color: '#fff', px: 2, py: 0.5 }}>
      ...
      <FullScreenDialog
        ...
        hideFab
        actions={<Tooltip title="Adicionar"><IconButton onClick={() => setDialogOpen(true)}><Add /></IconButton></Tooltip>}
      >
        {renderItemLista()}
        <DialogNovaEntrada open={dialogOpen} onClose={() => setDialogOpen(false)} onSave={salvarItem} novoItem={novoItem} setNovoItem={setNovoItem} />
        {renderRodape()}
      </FullScreenDialog>
    </Box>
  );
